...
#if CMSDK_GPIO_ALTFUNC_UART0_TX
            GPIO_MAP[CMSDK_GPIO_UART0_TX_GPIO_NUM]->ALTFUNCSET |=
                                        (1<<CMSDK_GPIO_ALTFUNC_UART0_TX_SET);
#endif
          }
          if((int)uart_rx != NC)
          {
            CMSDK_UART0->CTRL |= 0x02;  // RX enable
#if CMSDK_GPIO_ALTFUNC_UART0_RX
            GPIO_MAP[CMSDK_GPIO_UART0_RX_GPIO_NUM]->ALTFUNCSET |=
                                        (1<<CMSDK_GPIO_ALTFUNC_UART0_RX_SET);
#endif
          }
        }
        break;
